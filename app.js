...
app.post("/delete-multiple", (req, res) => {
  const fileIds = JSON.parse(req.body.fileIds); // Giải nén JSON từ body gửi lên

  if (!fileIds || fileIds.length === 0) {
    return res.redirect("/dashboard"); // Nếu không có file nào chọn, chuyển về dashboard
  }

  // Xoá từng file đã chọn
  fileIds.forEach(id => {
    db.get("SELECT path FROM files WHERE id = ?", [id], (err, row) => {
      ...
      if (row) {
        ...
        db.run("DELETE FROM files WHERE id = ?", [id], err => {
          if (err) console.error("Xoá DB lỗi:", err);
        });
      }
    });
  });

  res.redirect("/dashboard"); // Sau khi xoá xong, chuyển về dashboard
});
